...
class Game:
    ...
    def start_game(self):
        ...
        for i in range(4):
            ...
            self.win_button[i] = Button(root, text="WIN", state="disabled", font=("Helvetica", 15, "bold"), command=lambda i=i :self.player_wins(i))
            self.win_button[i].grid(row=i, column=0, padx=10, pady=10)
            self.player_button[i] = Button(root, text=name, font=("Helvetica", 24, "bold"), command=lambda i=i :self.player_loses(i))
            self.player_button[i].grid(row=i, column=1, padx=10, pady=10)
            self.score_label[i] = Label(root, text=f"{len(self.player_cards[i])} cards left", font=("Helvetica", 24, "bold"))
            self.score_label[i].grid(row=i, column=2, padx=10, pady=10)
        ...
        self.current_card_image.grid(row=0, column=3, rowspan=3, padx=10, pady=10)
        self.deck_label = Label(root, text="Deck: 0", font=("Helvetica", 24, "bold"), justify="center")
        self.deck_label.grid(row=3, column=3)
